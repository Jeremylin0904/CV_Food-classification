...
Original file is located at
    https://colab.research.google.com/drive/1Zh-goksy6b3fz1FknDb7-zeu48PIMXkN
"""
...
domainselector = tf.keras.models.load_model('/content/drive/Shareddrives/final_project/model(domainselection).model')
modelw = tf.keras.models.load_model('/content/drive/Shareddrives/final_project/model(web).model')
modelk = tf.keras.models.load_model('/content/drive/Shareddrives/final_project/model(Kaggle).model')
...
def predict(img):
  ...
  if domain[0]>0.5:
    ...
    df = pd.read_csv("/content/drive/Shareddrives/final_project/tw_food_101_classes.csv", names=column_names)
...
def finetune(img, label):
  ...
  if domain[0]>0.5:
    ...
    modelw.save('/content/drive/Shareddrives/final_project/model(web).model')
  if domain[0]<0.5:


    modelk.fit(np.array((x,x)),y)

    modelk.save('/content/drive/Shareddrives/final_project/model(Kaggle).model')
